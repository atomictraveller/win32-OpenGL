...
//	begin opengl
	...
	const char* vertexShaderSource = R"(
		...
		void main() {
			...
			texCoord = vec3(pos.x + 128.0, pos.y + 128.0, pos.z + 128.0);
			norm = (normal.x * .3535 + normal.y * .7071 + normal.z * .6123 - 0.7);// * 0.0;
			//norm *= abs(norm) * 4.0;
		}
	)";
	...
	const char* fragmentShaderSource = R"(
		...
		void main() {
			...
			for (int i = 0; i < octaves; i++) {
				...
				ix = int(hfx);	iy = int(hfy);	iz = int(hfz);
				float dx = hfx - ix;	float dy = hfy - iy;	float dz = hfz - iz;

				float r = 0.39999;
				hfx = ix;	hfy = iy;	hfz = iz;
				ix++;	iy++;	iz++;
				float x2 = ix;	float y2 = iy;	float z2 = iz;
				
				float vt0 = fract((hfx + hfy + hfz) * r);	//	vertices of cube for interpolation
				float vt1 = fract((x2  + hfy + hfz) * r);
				float vt2 = fract((hfx + hfy + z2) * r);
				float vt3 = fract((x2  + hfy + z2) * r);
				float vt4 = fract((hfx + y2  + hfz) * r);
				float vt5 = fract((x2  + y2  + hfz) * r);
				float vt6 = fract((hfx + y2  + z2) * r);
				float vt7 = fract((x2  + y2  + z2) * r);
...
FragColor = vec4(0.24 + hf, 0.15 + hf, hf, 1.0);
...
*/
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 9 * sizeof(float), (void*)0);	// Specify the vertex array pointer to the vertex data
